...
export class GeminiProvider implements Provider {
    ...
    for (const account of accounts) {
      try {
        ...
        if (data.buckets && data.buckets.length > 0) {
          ...
          const resetTimes = data.buckets.map(b => parseISO8601(b.resetTime)).filter((d): d is Date => d !== undefined);
          const earliestReset = resetTimes.length > 0 ? new Date(Math.min(...resetTimes.map(d => d.getTime()))) : undefined;
  ...
  private async getAccounts(auth: AuthConfig): Promise<GeminiAccount[]> {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        if (Array.isArray(parsed.accounts)) {
          return parsed.accounts as Array<{ projectId?: string; refreshToken: string }>;
        }
