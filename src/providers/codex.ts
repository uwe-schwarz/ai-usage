...
export class CodexProvider implements Provider {
	...
	async fetchUsage(auth: AuthConfig): Promise<ProviderUsage> {
		...
		try {
			...
			const primaryWindow: UsageWindow | undefined = data.rate_limit
				?.primary_window
				? {
						...
						resetAt: data.rate_limit.primary_window.reset_at
							...
							: new Date(
									Date.now() +
										data.rate_limit.primary_window.reset_after_seconds * 1000,
								),
					}
				: undefined;

			const secondaryWindow: UsageWindow | undefined = data.rate_limit
				?.secondary_window
				? {
						...
						resetAt: data.rate_limit.secondary_window.reset_at
							...
							: new Date(
									Date.now() +
										data.rate_limit.secondary_window.reset_after_seconds * 1000,
								),
					}
				: undefined;
