	...
	async fetchUsage(auth: AuthConfig): Promise<ProviderUsage> {
		...
		try {
			...
			const fiveHourWindow: UsageWindow | undefined = data.five_hour
				? {
						...
						resetAt: data.five_hour.resets_at
							? parseISO8601(data.five_hour.resets_at)
							: undefined,
					}
				: undefined;
			...
			const weeklyWindow: UsageWindow | undefined = data.seven_day
				? {
						...
						resetAt: data.seven_day.resets_at
							? parseISO8601(data.seven_day.resets_at)
							: undefined,
					}
				: undefined;
			...
			if (data.seven_day_sonnet) {
				additionalInfo.push(
					`Sonnet: ${data.seven_day_sonnet.utilization.toFixed(1)}%`,
				);
			}
			if (data.seven_day_opus) {
				additionalInfo.push(
					`Opus: ${data.seven_day_opus.utilization.toFixed(1)}%`,
				);
			}
