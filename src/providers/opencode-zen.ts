...
export class OpenCodeZenProvider implements Provider {
  ...
  private async runOpenCodeStats(binaryPath: string): Promise<OpenCodeStats | null> {
    try {
      const { stdout } = await execAsync(`"${binaryPath}" stats --days 7`, { timeout: 30000 });
  ...
  private parseStats(output: string): OpenCodeStats | null {
    try {
      ...
      const sessions = sessionsMatch ? parseInt(sessionsMatch[1].replace(/,/g, ''), 10) : 0;

      // Parse Messages
      const messagesMatch = output.match(/│Messages\s+([0-9,]+)/);
      const messages = messagesMatch ? parseInt(messagesMatch[1].replace(/,/g, ''), 10) : 0;
